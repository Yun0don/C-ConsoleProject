...
namespace RogueLikeRpg
{
    public class DungeonManager
    {
        ...
        public void GoDown(Player player)
        {
            ...
            {
                // 새 층 생성
                Dungeon newFloor = new Dungeon();
                // 이전 층의 내려가는 포탈 위치를 새 층의 위 포탈('^')로 강제 설정
                newFloor.SetUpPortal(currentFloor.DownPortal);
                // 새 층에 내려가는 포탈('v') 배치
                newFloor.PlaceDownPortal();
                dungeonStack.Push(newFloor);
                // 플레이어 위치를 새 층의 위 포탈 위치로 변경
        ...
        public void GoUp(Player player)
        {
            ...
            if (player.X == currentFloor.UpPortal.x && player.Y == currentFloor.UpPortal.y)
            {
                ...
                // 플레이어 위치를 이전 층의 내려가는 포탈('v') 위치로 이동
